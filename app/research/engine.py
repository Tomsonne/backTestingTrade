...
def _build_candidates(
    ...
    if config.validation.mode == "trace":
        ...
        for candidate in candidates:
            previous, current = contexts[(candidate.trade_date, candidate.session)]
            annotate_candidate(candidate, catalog, previous, current,
                               trigger=config.trigger.mode != "NONE", zones=bool(config.zones.timeframes),
                               dxy=config.divergence.enabled, history_index=mid.index,
                               zone_timeframes=config.zones.timeframes)
        for setup in rejected:
            previous, current = contexts[(setup["trade_date"], setup["session"])]
            at = pd.Timestamp(setup.get("entry_time") or setup.get("label_time") or setup.get("break_time") or current.end)
            if setup.get("break_time") and not setup.get("entry_time"):
                at += MINUTE
            events = setup_events(catalog, pair, previous, current, at,
                                  trigger=config.trigger.mode != "NONE" and setup["reason"] in {"NO_LABEL", "LABEL_TOO_LATE", "NO_ZONE", "ZONE_WRONG_DIRECTION", "ENTRY_OUTSIDE_SESSION"},
                                  zones=bool(config.zones.timeframes) and setup["reason"] in {"NO_ZONE", "ZONE_WRONG_DIRECTION"},
                                  dxy=config.divergence.enabled and setup["reason"] != "NO_PAIR_BREAK",
                                  label_time=setup.get("label_time"), history_index=mid.index)
            status = "INDETERMINATE" if setup["reason"] in {"DXY_DATA_MISSING", "PREVIOUS_SESSION_DATA_MISSING"} else None
...
def run_research_backtest(
    ...
    if config.validation.mode == "trace":
        runtime.gap_catalog.dxy_symbols = (
            ...
            else list(DXY_COMPONENTS) if dxy_source != "disabled" else []
        )
        warnings = [{"instrument": symbol, "rows": report["rows"],
                     "unexpected_missing_minutes": report["unexpected_missing_minutes"],
