...
- `trace` : continue avec les vraies bougies disponibles, annote les dépendances du setup et documente les hypothèses de gestion des positions. Aucune interpolation ni bougie de remplacement. Les prix/OHLC corrompus restent bloquants.
- `permissive` : compatibilité avec les anciennes expériences, sans garantie de traçabilité détaillée.

TRACE attribue `COMPLETE`, `DEGRADED`, `GAP_RESOLVED` ou `INDETERMINATE`. Après un trou pendant une position, **l'OPEN exécutable de la première bougie réelle** est prioritaire : au-delà du TP → TP inféré; au-delà du SL → SL inféré; entre les deux → hypothèse `ASSUMED_NO_EXIT`, puis application de la politique OHLC habituelle (`stop_first`/`tp_first`). BUY sort au BID, SELL à l'ASK. L'heure enregistrée pour une sortie inférée est celle de l'observation, pas une heure de toucher inventée. Sans observation exploitable dans la fenêtre de détention, le résultat est indéterminé, sans R ni PnL numériques.

Le détail du run affiche **ALL / CLEAN** et le journal des trades comporte un filtre qualité, un badge et tous les événements. ALL mesure les trades déterminés, y compris les hypothèses; CLEAN ne garde que COMPLETE et ne rejoue pas la sélection quotidienne. Les trades indéterminés restent visibles mais sont exclus des performances. Les runs antérieurs restent « non évalués » : ils ne deviennent pas COMPLETE rétroactivement.

Les indicateurs récursifs, la mémoire des pivots et les anciennes zones peuvent dépendre de tout l'historique chargé. TRACE conserve cette dépendance de façon prudente : un trou ancien peut dégrader un setup récent, et CLEAN peut être vide. Les fermetures de marché attendues ne sont pas des trous. Une session précédente située hors de la période demandée reste un contexte non chargé; TRACE ne télécharge pas automatiquement de warm-up.

```bash
python scripts/run_research.py --preset "Legacy A.0" --from 2026-02-01 --to 2026-08-25 --validation trace --name "Recherche TRACE"
```
